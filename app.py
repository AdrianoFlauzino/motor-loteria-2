...
with tab2:
    ...
    fig_mc = px.bar(
        x=range(1, MAX_NUM+1),
        y=mc_freq,
        title=f"Frequência em {n_montecarlo:,} Simulações"
    )
...
with tab3:
    ...
    cost = st.number_input("Custo por aposta (R$)", 5.0)
    prize = st.number_input("Prêmio estimado Sena (R$)", 50_000_000.0)

    prob_win = 1 / total_combos
    ev = prize * prob_win - cost
    roi = (ev / cost) * 100
    payoff = prize / cost

    col1.metric("EV", f"R$ {ev:,.2f}")
    col2.metric("ROI", f"{roi:.6f}%")
...
with tab4:
    ...
    try:
        ...
        st.info(f"{len(combs):,} combinações geradas.")
        ...
        if len(combs) <= 500:
            ...
            st.warning("Mostrando apenas as 100 primeiras.")
    except:
        st.error("Erro: entrada inválida.")
...
with tab5:
    ...
    c1.metric("Total Sorteios", f"{len(historical_df):,}")
    c2.metric("Soma Média", f"{sums.mean():.1f}")
    c3.metric("Desvio Padrão", f"{sums.std():.1f}")
    c4.metric("Hot Máx", f"{hist_freq.max()}")

    c5, c6, c7, c8 = st.columns(4)
    c5.metric("Simulações", f"{n_montecarlo:,}")
    c6.metric("Média MC", f"{mc_freq.mean():.1f}")
    c7.metric("Combos Totais", f"{total_combos:,}")
    c8.metric("EV Ref", f"R$ {(50_000_000*(1/total_combos) - 5):,.4f}")
